// Generated by zotero-plugin-scaffold
/* prettier-ignore */
/* eslint-disable */
// @ts-nocheck
export type FluentMessageId =
  | 'anno-annotation'
  | 'anno-comment'
  | 'greek-default'
  | 'greek-fixed'
  | 'greek-webSpeech'
  | 'itemMenu-abstract'
  | 'itemMenu-title'
  | 'popup-SFH-noSelection'
  | 'popup-SFH-nonspecificSelection1'
  | 'popup-SFH-nonspecificSelection2'
  | 'popup-SFH-unknownSelection1'
  | 'popup-SFH-unknownSelection2'
  | 'popup-addonErrorTitle'
  | 'popup-addonLoadingTitle'
  | 'popup-allGood'
  | 'popup-engineErrorCause'
  | 'popup-engineErrorTitle'
  | 'popup-engineLoadingTitle'
  | 'popup-errorTrailer'
  | 'popup-faveLoaded-body'
  | 'popup-faveLoaded-noneSet'
  | 'popup-faveLoaded-title'
  | 'popup-loadingTrailer'
  | 'pref-azure-key'
  | 'pref-azure-language'
  | 'pref-azure-minSegmentSize'
  | 'pref-azure-rate'
  | 'pref-azure-region'
  | 'pref-azure-testText'
  | 'pref-azure-testVoice'
  | 'pref-azure-voice'
  | 'pref-azure-volume'
  | 'pref-faves-addVoice'
  | 'pref-faves-list'
  | 'pref-faves-p1'
  | 'pref-faves-removeVoice'
  | 'pref-general-engine-azure'
  | 'pref-general-engine-kokoro'
  | 'pref-general-engine-label'
  | 'pref-general-engine-local'
  | 'pref-general-engine-openai'
  | 'pref-general-engine-webSpeech'
  | 'pref-general-queue-disable'
  | 'pref-general-queue-enable'
  | 'pref-general-queue-label'
  | 'pref-general-reloadTabs'
  | 'pref-ignoreAnnotations-color'
  | 'pref-ignoreAnnotations-description'
  | 'pref-kokoro-apiUrl'
  | 'pref-kokoro-language'
  | 'pref-kokoro-model'
  | 'pref-kokoro-rate'
  | 'pref-kokoro-testText'
  | 'pref-kokoro-testVoice'
  | 'pref-kokoro-voice'
  | 'pref-kokoro-volume'
  | 'pref-local-apiUrl'
  | 'pref-local-model'
  | 'pref-local-rate'
  | 'pref-local-testText'
  | 'pref-local-testVoice'
  | 'pref-local-voice'
  | 'pref-local-volume'
  | 'pref-openai-key'
  | 'pref-openai-model'
  | 'pref-openai-rate'
  | 'pref-openai-testText'
  | 'pref-openai-testVoice'
  | 'pref-openai-voice'
  | 'pref-openai-volume'
  | 'pref-sect-advanced'
  | 'pref-sect-azure'
  | 'pref-sect-general'
  | 'pref-sect-ignoreAnnotations'
  | 'pref-sect-kokoro'
  | 'pref-sect-local'
  | 'pref-sect-openai'
  | 'pref-sect-shortcuts'
  | 'pref-sect-wsa'
  | 'pref-shiftMod-p1'
  | 'pref-shiftMod-p2'
  | 'pref-shiftMod-swapAnno'
  | 'pref-shiftMod-swapAnno-false'
  | 'pref-shiftMod-swapAnno-true'
  | 'pref-shiftMod-swapLib'
  | 'pref-shiftMod-swapLib-false'
  | 'pref-shiftMod-swapLib-true'
  | 'pref-shiftMod-swapSpeakSelection'
  | 'pref-shiftMod-swapSpeakSelection-false'
  | 'pref-shiftMod-swapSpeakSelection-true'
  | 'pref-shortcuts-cancel'
  | 'pref-shortcuts-cycleFav'
  | 'pref-shortcuts-p1'
  | 'pref-shortcuts-pause'
  | 'pref-shortcuts-speak'
  | 'pref-shortcuts-speedDown'
  | 'pref-shortcuts-speedUp'
  | 'pref-status-allGood'
  | 'pref-status-error'
  | 'pref-status-unknown'
  | 'pref-subs-citationsNumeric'
  | 'pref-subs-citationsOverall'
  | 'pref-subs-citationsParenthetical'
  | 'pref-subs-emails'
  | 'pref-subs-greekLetters'
  | 'pref-subs-p1'
  | 'pref-subs-p2'
  | 'pref-subs-placeholder'
  | 'pref-subs-urls'
  | 'pref-subs-warning'
  | 'pref-subsect-advanced-faves'
  | 'pref-subsect-advanced-subs'
  | 'pref-subsect-shortcuts-keys'
  | 'pref-subsect-shortcuts-shiftMod'
  | 'pref-wsa-pitch'
  | 'pref-wsa-rate'
  | 'pref-wsa-testVoice'
  | 'pref-wsa-voice'
  | 'pref-wsa-volume'
  | 'speak-testVoice'
  | 'textPopup-fromHere'
  | 'textPopup-selection'
  | 'ttsEngine-engineName'
  | 'ttsEngine-errorCause'
  | 'ttsEngine-settingsFormatted';
